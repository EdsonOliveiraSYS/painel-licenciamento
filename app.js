...
const countryName=value=>countries[String(value||'BR').toUpperCase()]||String(value||'BR').toUpperCase();
const countryOptions=(selected='BR')=>Object.entries(countries).map(([code,name])=>`<option value="${code}" ${code===(selected||'BR')?'selected':''}>${name}</option>`).join('');
const fiscalId=item=>item?.tax_id||item?.cnpj||'';
...
function applyCentralTheme(theme,{persist=true}={}){const selectedTheme=theme==='light'?'light':'dark';document.documentElement.dataset.theme=selectedTheme;if(persist){try{localStorage.setItem(CENTRAL_THEME_KEY,selectedTheme);}catch(_){}}const button=$('themeToggle');if(button){const light=selectedTheme==='light';button.setAttribute('aria-pressed',String(!light));button.innerHTML=light?'☀ <span>Tema escuro</span>':'☾ <span>Tema claro</span>';}}
function restoreCentralTheme(){let saved='';try{saved=localStorage.getItem(CENTRAL_THEME_KEY)||'';}catch(_){}applyCentralTheme(saved||'dark',{persist:false});}
...
async function loadPartners(renderAfter=true){
  ...
  if(renderAfter){renderPartnerOptions();renderTeamPartnerOptions();renderPartners();}
}
...
function renderTeamPartnerOptions(){const field=$('teamInvitePartner');if(!field)return;field.innerHTML=`<option value="">Sem parceiro — acesso geral</option>${partners.filter(item=>item.active).map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join('')}`;}
async function loadDownloads(){downloads=await api('/rest/v1/license_downloads?select=id,title,description,download_url,platform,active,created_at&order=created_at.desc')||[];renderDownloads();}
function renderDownloads(){const target=$('downloadsList');if(!target)return;target.innerHTML=`<div class="section-card-head"><div><h3>Links disponíveis</h3><p>Arquivos oficiais indicados pela Central.</p></div></div>${downloads.length?downloads.map(item=>`<article class="release-row"><div><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.description||'Sem descrição.')}</p><small>${escapeHtml(item.platform)} · ${escapeHtml(item.download_url)}</small></div><div class="release-actions"><a class="button secondary compact" href="${escapeHtml(item.download_url)}" target="_blank" rel="noopener">Abrir link</a></div></article>`).join(''):'<div class="release-empty">Nenhum download cadastrado.</div>'}`;}
async function saveDownload(event){event.preventDefault();const title=$('downloadTitle').value.trim(),url=$('downloadUrl').value.trim(),button=$('downloadSaveButton');$('downloadError').textContent='';if(!/^https:\/\//i.test(url)){$('downloadError').textContent='Use um link HTTPS.';return;}button.disabled=true;try{await api('/rest/v1/license_downloads',{method:'POST',body:{title,download_url:url,platform:$('downloadPlatform').value,description:$('downloadDescription').value.trim()||null,created_by:session.user.id}});event.target.reset();showToast('Download cadastrado.');await loadDownloads();}catch(error){$('downloadError').textContent=error.message;}finally{button.disabled=false;}}
...
async function inviteTeamMember(event){
  ...
  try{await teamApi('invite',{fullName:name,email,partnerId:$('teamInvitePartner').value||null});event.target.reset();showToast('Convite enviado por e-mail.');await loadTeam();}
...
function render(){
  ...
  const filtered=installations.filter(item=>{const academy=item.academies||{};return inSelectedView(item)&&(!status||item.status===status)&&(!query||`${academy.name||''} ${fiscalId(academy)} ${item.installation_id}`.toLowerCase().includes(query));});
...
function printPartnerReport(partnerId){
  ...
  const bounds=monthBounds($('financeMonth').value)||monthBounds(currentMonthIso()),statusLabels={pending:'Pendente',paid:'Paga',overdue:'Vencida',waived:'Isenta',cancelled:'Cancelada'};
  const charges=financialCharges.filter(item=>item.partner_id===partnerId&&item.due_date>=bounds.start&&item.due_date<bounds.end),total=charges.filter(item=>!['cancelled','waived'].includes(item.status)).reduce((sum,item)=>sum+Number(item.amount_cents||0),0);
  const payload={partner:{name:partner.name,cnpj:partner.cnpj||''},period:{start:formatDateOnly(bounds.start),end:formatDateOnly(new Date(new Date(`${bounds.end}T00:00:00`).getTime()-86400000).toISOString().slice(0,10))},rows:charges.map(item=>({academy:installations.find(entry=>entry.academy_id===item.academy_id)?.academies?.name||'Academia',due:formatDateOnly(item.due_date),status:statusLabels[item.status]||item.status,amount:formatMoneyCents(item.amount_cents)})),total:formatMoneyCents(total),generatedAt:formatDate(new Date().toISOString())};
  const bytes=new TextEncoder().encode(JSON.stringify(payload));let binary='';bytes.forEach(byte=>binary+=String.fromCharCode(byte));const encoded=btoa(binary).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
  const win=window.open(`report.html#${encoded}`,'_blank');if(!win){showToast('Permita janelas pop-up para gerar o PDF.',true);return;}win.opener=null;
}
...
function renderDelinquencies(total){
  ...
  $('delinquencyList').innerHTML=`<div class="table-scroll"><table><thead><tr><th>Academia</th><th>Documento fiscal</th><th>WhatsApp</th><th>Vencimento</th><th>Valor</th><th>Ações</th></tr></thead><tbody>${delinquencies.map(({charge,license,academy})=>`<tr><td><strong>${escapeHtml(academy.name||'Academia')}</strong><span>${escapeHtml(academy.legal_name||'Razão social não informada')}</span></td><td>${escapeHtml(fiscalId(academy)||'—')}<span>${escapeHtml(countryName(academy.country_code))}</span></td><td>${escapeHtml(academy.phone||'—')}</td><td>${formatDateOnly(charge.due_date)}</td><td><strong>${formatMoneyCents(charge.amount_cents)}</strong></td><td><div class="row-actions"><button class="button secondary compact" data-email="${license?.id||charge.license_id}" type="button" ${emailProviderConfigured&&academy.email?'':'disabled'}>Enviar e-mail</button><button class="button secondary compact" data-paid="${charge.id}" type="button">Marcar como paga</button></div></td></tr>`).join('')}</tbody></table></div>`;
}
...
async function saveBilling(){
  ...
  try{const countryCode=$('billingAcademyCountry').value,taxId=$('billingAcademyTaxId').value.trim()||null,identity={name:$('billingAcademyName').value.trim(),country_code:countryCode,tax_id:taxId,cnpj:countryCode==='BR'?(taxId||'').replace(/\D/g,'').slice(0,14)||null:null,legal_name:$('billingAcademyLegalName').value.trim()||null,phone:$('billingAcademyPhone').value.trim()||null,email:$('billingAcademyEmail').value.trim().toLowerCase()||null,updated_at:new Date().toISOString()};if(!identity.name){throw new Error('Informe o nome da academia.');}await api(`/rest/v1/academies?id=eq.${encodeURIComponent(editingBilling.installation.academy_id)}`,{method:'PATCH',body:identity});await api(`/rest/v1/installations?id=eq.${encodeURIComponent(editingBilling.installation.id)}`,{method:'PATCH',body:{environment:$('billingInstallationEnvironment').value}});const partnerId=$('billingPartner').value||null;await api(`/rest/v1/licenses?id=eq.${encodeURIComponent(editingBilling.license.id)}`,{method:'PATCH',body:{partner_id:partnerId,billing_payer:partnerId?'partner':'academy'}});await api(`/rest/v1/license_charges?license_id=eq.${encodeURIComponent(editingBilling.license.id)}`,{method:'PATCH',body:{partner_id:partnerId,updated_at:new Date().toISOString()}});const result=await api('/functions/v1/license-billing-update',{method:'POST',body:billingPayload(editingBilling.license,{billingCycle:$('billingEditCycle').value,billingAmountCents:Math.round(amount*100),billingDueDate:amount>0?dueDate:null,billingStatus:amount>0?status:'waived',billingCollectionMode:$('billingEditCollectionMode').value,billingNoticeEnabled:$('billingEditNoticeEnabled').checked,billingNoticeDays:noticeDays,billingNotificationChannel:$('billingEditChannel').value,billingEnforcementMode:$('billingEditEnforcementMode').value,billingGraceDays:graceDays})});editingBilling=null;$('billingModal').classList.add('hidden');showToast(result.renewed?'Pagamento registrado e próximo vencimento gerado.':'Dados da academia, plano, avisos e automações atualizados.');await loadInstallations();}catch(error){$('billingEditError').textContent=error.message;}finally{savingBilling=false;$('billingSaveButton').disabled=false;$('billingSaveButton').textContent='Salvar plano e cobrança';}
}
...
async function issue(){
  ...
  }catch(error){$('modalError').textContent=error.message==='active_license_exists'?'Esta instalação já possui uma licença ativa. Use “Editar licença” ou exclua a licença atual antes de emitir outra.':error.message;}
...
function exportDelinquencies(){
  ...
  const rows=[['Nome','Documento fiscal','País','Razão Social','Número WhatsApp','E-mail','Data Vencimento','Valor'],...delinquencies.map(({charge,academy})=>[academy.name||'',fiscalId(academy),countryName(academy.country_code),academy.legal_name||'',academy.phone||'',academy.email||'',formatDateOnly(charge.due_date),formatMoneyCents(charge.amount_cents)])];
...
async function deleteInstallation(id){
  const item=installations.find(row=>row.id===id);if(!item)return;
  const academy=item.academies?.name||'Academia sem nome';
  if(!['inactive','blocked'].includes(item.status)){showToast('Por segurança, inative ou bloqueie a instalação antes de excluí-la.',true);return;}
  if(!confirm(`Excluir definitivamente a instalação ${academy}?\n\nSerão removidos também as licenças, cobranças e eventos ligados somente a esta instalação. Esta ação não pode ser desfeita.`))return;
  try{await api(`/rest/v1/installations?id=eq.${encodeURIComponent(id)}`,{method:'DELETE'});showToast('Instalação e registros vinculados excluídos.');await loadInstallations();}
...
function openPartnerModal(id=''){
  const partner=partners.find(item=>item.id===id)||null;$('partnerId').value=partner?.id||'';$('partnerModalTitle').textContent=partner?'Editar parceiro':'Novo parceiro';$('partnerName').value=partner?.name||'';$('partnerCountry').innerHTML=countryOptions(partner?.country_code||'BR');$('partnerTaxId').value=fiscalId(partner);$('partnerDefaultAmount').value=(Number(partner?.default_amount_cents??7000)/100).toFixed(2);$('partnerResponsible').value=partner?.responsible_name||'';$('partnerPhone').value=partner?.phone||'';$('partnerEmail').value=partner?.email||'';$('partnerNotes').value=partner?.notes||'';$('partnerActive').checked=partner?.active!==false;$('partnerError').textContent='';$('partnerModal').classList.remove('hidden');$('partnerName').focus();
}
...
async function savePartner(){
  ...
  const countryCode=$('partnerCountry').value,taxId=$('partnerTaxId').value.trim()||null,body={name,country_code:countryCode,tax_id:taxId,cnpj:countryCode==='BR'?(taxId||'').replace(/\D/g,'').slice(0,14)||null:null,default_amount_cents:Math.round(amount*100),responsible_name:$('partnerResponsible').value.trim()||null,phone:$('partnerPhone').value.trim()||null,email:$('partnerEmail').value.trim().toLowerCase()||null,notes:$('partnerNotes').value.trim()||null,active:$('partnerActive').checked,updated_at:new Date().toISOString()},button=$('partnerSaveButton');button.disabled=true;
