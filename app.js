...
async function saveBilling(){
  ...
  try{await api('/functions/v1/license-billing-update',{method:'POST',body:billingPayload(editingBilling.license,{billingCycle:$('billingEditCycle').value,billingAmountCents:Math.round(amount*100),billingDueDate:amount>0?dueDate:null,billingStatus:amount>0?status:'waived',billingCollectionMode:$('billingEditCollectionMode').value,billingNoticeEnabled:$('billingEditNoticeEnabled').checked,billingNoticeDays:noticeDays,billingNotificationChannel:$('billingEditChannel').value,billingEnforcementMode:$('billingEditEnforcementMode').value,billingGraceDays:graceDays})});editingBilling=null;$('billingModal').classList.add('hidden');showToast('Plano, avisos e automações atualizados.');await loadInstallations();}catch(error){$('billingEditError').textContent=error.message;}finally{savingBilling=false;$('billingSaveButton').disabled=false;$('billingSaveButton').textContent='Salvar plano e cobrança';}
}
...
async function markPaid(licenseId){
  ...
  try{if(!item)throw new Error('Cobrança não encontrada.');await api('/functions/v1/license-billing-update',{method:'POST',body:billingPayload(item.license,{billingStatus:'paid'})});showToast('Cobrança marcada como paga e bloqueio financeiro removido.');await loadInstallations();}catch(error){showToast(error.message,true);}
}
