...
function printPartnerReport(partnerId){
  ...
  const bounds=monthBounds($('financeMonth').value)||monthBounds(currentMonthIso());const charges=financialCharges.filter(item=>item.partner_id===partnerId&&item.due_date>=bounds.start&&item.due_date<bounds.end);const rows=charges.map(item=>`<tr><td>${escapeHtml(installations.find(entry=>entry.academy_id===item.academy_id)?.academies?.name||'Academia')}</td><td>${formatDateOnly(item.due_date)}</td><td>${escapeHtml({pending:'Pendente',paid:'Paga',overdue:'Vencida',waived:'Isenta',cancelled:'Cancelada'}[item.status]||item.status)}</td><td>${formatMoneyCents(item.amount_cents)}</td></tr>`).join('')||'<tr><td colspan="4">Nenhuma cobrança no período.</td></tr>';const total=charges.filter(item=>!['cancelled','waived'].includes(item.status)).reduce((sum,item)=>sum+Number(item.amount_cents||0),0);const win=window.open('','_blank');if(!win){showToast('Permita janelas pop-up para gerar o PDF.',true);return;}win.opener=null;win.document.write(`<!doctype html><html lang="pt-BR"><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Relatório ${partner.name} ${bounds.start}</title><style>body{font:14px Arial;color:#182033;padding:38px}h1{margin:0}p{color:#66758a}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{border-bottom:1px solid #dfe4eb;padding:10px;text-align:left}th{background:#f2f4f7}.total{margin-top:20px;font-size:18px;font-weight:bold}@media(max-width:640px){body{padding:18px;font-size:12px}th,td{padding:7px 4px}}@media print{body{padding:0}}</style></head><body><h1>ZentryxFit · Relatório de licenças</h1><p><strong>Parceiro:</strong> ${escapeHtml(partner.name)}${partner.cnpj?` · CNPJ ${escapeHtml(partner.cnpj)}`:''}<br><strong>Período:</strong> ${formatDateOnly(bounds.start)} a ${formatDateOnly(new Date(new Date(`${bounds.end}T00:00:00`).getTime()-86400000).toISOString().slice(0,10))}</p><table><thead><tr><th>Academia</th><th>Vencimento</th><th>Situação</th><th>Valor ZentryxFit</th></tr></thead><tbody>${rows}</tbody></table><p class="total">Total previsto: ${formatMoneyCents(total)}</p><p>Documento gerado em ${formatDate(new Date().toISOString())}.</p><script>window.addEventListener('load',()=>setTimeout(()=>window.print(),150))<\/script></body></html>`);win.document.close();win.focus();}
