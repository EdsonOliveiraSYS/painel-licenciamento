...
let session=null,installations=[],financialCharges=[],delinquentCharges=[],messageTemplates=[],emailDeliveries=[],appReleases=[],partners=[],emailProviderConfigured=false,selected=null,issuing=false,editingBilling=null,savingBilling=false,savingTemplate=false,sendingEmail=false,publishingUpdate=false,delinquencies=[],clientView='active',centralPanel='overview',installationQrReader=null;
...
async function loadPartners(renderAfter=true){
  try{partners=await api('/rest/v1/license_partners?select=id,name,cnpj,responsible_name,phone,email,default_amount_cents,active,notes,created_at,updated_at&order=name.asc')||[];}
  catch(error){partners=[];console.warn('partners unavailable',error.message);}
